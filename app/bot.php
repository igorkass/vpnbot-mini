...
class Bot
{
    ...
    public function action()
    {
        switch (true) {
            ...
            case preg_match('~^/menu (?P<type>pac|adguard|config|lang|mirror|update)$~', $this->input['callback'], $m):
    ...
    public function checkVersion()
    {
        try {
            ...
            if (!empty($c['admin']) && (empty($this->time) || ((time() - $this->time) > 3600))) {
                ...
                $last       = file_get_contents("https://raw.githubusercontent.com/igorkass/vpnbot-mini/$b/version");
                        ...
                        foreach ($c['admin'] as $k => $v) {
                            $this->send($v, implode("\n", $diff), 0, [
                                [
                                    [
                                        'text'    => 'changelog',
                                        'web_app' => ['url' => "https://raw.githubusercontent.com/igorkass/vpnbot-mini/$b/version"],
                                    ],
                                    [
...
<code>example.org https://dns.adguard.com/dns-query 94.140.14.14</code>",
    ...
    public function subnet($wgpage = 0, $page = 0)
    {
        $count  = $this->limit;
        ...
        if (!empty($subnets)) {
            ...
            if ($page != -1 && $all > 1) {
                $data[] = [
                    ...
                    [
                        'text'          => $page + 1,
                        'callback_data' => "/subnet {$wgpage}_$page",
                    ],
                    [
                        'text'          => '>>',
    ...
    public function cleanDocker()
    {
        ...
        foreach ($r as $v) {
            if (!empty($v['RepoTags'])) {
                foreach ($v['RepoTags'] as $j) {
                    if (preg_match('~^mercurykd/vpnbot|^igorkass/vpnbot-mini~', $j)) {
                        $i[] = $v['Id'];
                        break;
                    }
                }
        ...
        foreach ($r as $v) {
            if (preg_match('~^mercurykd/vpnbot|^igorkass/vpnbot-mini~', $v['Image'])) {
                $c[] = $v['ImageID'];
            }
        }
